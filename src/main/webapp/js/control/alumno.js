...
var control_alumno_list = function(path) {
    ...
    function loadModalForm(view, place, id, action) {
        ...
        //clave ajena tutor
        cargaClaveAjena('#id_tutor', '#id_tutor_desc', 'tutor')
        $(prefijo_div + '#id_tutor_button').unbind('click');
        $(prefijo_div + '#id_tutor_button').click(function() {
            loadForeign('tutor', '#modal02', control_tutor_list, callbackSearchTutor);
            function callbackSearchTutor(id) {
                $(prefijo_div + '#modal02').modal('hide');
                $(prefijo_div + '#modal02').data('modal', null);
                $(prefijo_div + '#id_tutor').val($(this).attr('id'));
                cargaClaveAjena('#id_tutor', '#id_tutor_desc', 'tutor');
                return false;
            }
            return false;
